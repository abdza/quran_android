...
class RecentPagePresenterTest {
  ...
  fun `unbind persists the latest visited page`() = runTest {
    ...
    val presenter = RecentPagePresenter(recentPagesDao, LatestPageTracker(), ReadingSessionManager(FakeBookmarksDao()), "madani")
  ...
  fun `page changes update latest page before persistence`() = runTest {
    ...
    val presenter = RecentPagePresenter(RecordingRecentPagesDao(), tracker, ReadingSessionManager(FakeBookmarksDao()), "madani")
